...
use concordium_std::{PublicKeyEd25519, SchemaType, Serialize, SignatureEd25519};
...
pub type ContractBalanceOfQueryParams = BalanceOfQueryParams<TokenIdU8>;
...
fn test_withdraw_cis2_tokens() {
    ...
    assert_eq!(token_balance_of_bob_before.0, [TokenAmountU256(U256::zero())]);
    ...
    assert_eq!(token_balance_of_bob_after.0, [TokenAmountU256(withdraw_amount.0)]);
...
fn initialize_chain_and_contract() -> (Chain, ContractAddress, ContractAddress) {
    ...
    let module =
        module_load_v1("tests/out/module.wasm.v1").expect("Module exists");
    let deployment =
